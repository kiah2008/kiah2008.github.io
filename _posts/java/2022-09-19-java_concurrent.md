...
categories: [java]
tags: [java, thread]
description: 介绍java多线程知识点
keywords: java, thread, volatile, synchronized
dashang: true
topmost: false
mermaid: true
...
<div style=" background:black; font:normal 14px/24px 'MicroSoft YaHei';color:rgb(255,255,255);">
// 结果总是小于1000
</div>
## 并发出现问题的根源: 并发三要素
...
**在Java中(区别与C/C++)，对基本数据类型的变量的读取和赋值操作是原子性操作**，即这些操作是不可被中断的，要么执行，要么不执行。 请分析以下哪些操作是原子性操作：
...
在Java里面，可以通过volatile关键字来保证**一定的“有序性”**（具体原理在下一节讲述）。另外可以通过synchronized和Lock来保证有序性，很显然，synchronized和Lock保证每个时刻是有一个线程执行同步代码，相当于是让线程顺序执行同步代码，自然就保证了有序性。当然JMM是通过Happens-Before 规则来保证有序性的。
...
![image](/images/java/single-thread-rule.png)
...
![image](/images/java/monitor-lock-rule.png)
...
![image](/images/java/thread-start-rule.png)
...
![image](/images/java/thread-join-rule.png)
...
线程兼容是指对象本身并不是线程安全的，但是可以通过在调用端正确地使用同步手段来保证对象在并发环境中可以安全地使用，我们平常说一个类不是线程安全的，绝大多数时候指的是这一种情况。Java API 中大部分的类都是属于线程兼容的，如与前面的 **Vector 和 HashTable(支持线程同步,适用于多线程) 相对应的集合类 ArrayList 和 HashMap** 等。
...
![image](/images/java/3646544a-cb57-451d-9e03-d3c4f5e4434a.png)
